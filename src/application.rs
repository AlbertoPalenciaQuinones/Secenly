...
pub fn run() -> Result<(), LicenseError> {
    ...
    loop {
        ...
        match opt.as_str() {
            ...
            "5" | "obtain_license" => {
                if let Err(e) = obtain_license(&conn) {
                    eprintln!("Error: {}", e);
                }
            }
            "6" | "obtain_seed" => {
                if let Err(e) = obtain_seed(&conn) {
                    eprintln!("Error: {}", e);
                }
            }
            "0" | "exit" => {
...
pub fn connect_db() -> Result<Connection, LicenseError> {
...
fn create_license(conn: &Connection) -> Result<(), LicenseError> {
    ...
    let seed_path = ask_path(&"seed is locate or where you want to save (with his name)".to_string());
...
fn modify_license(conn: &Connection) -> Result<(), LicenseError> {
    ...
    match option {
        // Fecha de expiración
        0 => {
            let input = ask_string("New expiration (DD/MM/YY):");

            let date = NaiveDate::parse_from_str(&input, "%d/%m/%y")?;

            license.expiration_date = DateTime::<Utc>::from_naive_utc_and_offset(
            date.and_hms_opt(0, 0, 0)
            .ok_or(LicenseError::InvalidDate("Invalid time".into()))?,
            Utc);
        }
        // Intervalo del latido
        1 => {
            license.heartbeat_interval = ask_i32("New heartbeat:");
        }
    ...
    let license_path = ask_path(&"license will be save".to_string());
...
fn obtain_license(conn: &Connection) -> Result<(), LicenseError> { 
    // Obtener la ruta donde se almacenará la licencia
    let license_path = ask_path(&"where the license will be saved".to_string())
        .join("license.der");
...
fn obtain_seed(conn: &Connection) -> Result<(), LicenseError> {
    // Obtener la ruta donde se almacenará la semilla
    let seed_path = ask_path(&"where the seed will be saved".to_string())
        .join("seed.dat");
