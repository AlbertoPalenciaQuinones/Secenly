...
use secenly::services::{hardware_manager::Hwid, license_manager::LicenseIdentifier};
...
mod tests_it3 {
    ...
    fn asn1_id_is_string_128_chars() {
        ...
        let mut product_id = Hwid::obtain_hwid().expect("ERROR");

        for byte in seed_bytes.iter() {
            product_id += &format!("{}", *byte as u32);
        }
        let license_identifier = LicenseIdentifier::initialize(&product_id).expect("ERROR");

        let license = LicenseAsn1 {
            id: license_identifier.get_license_id(),
