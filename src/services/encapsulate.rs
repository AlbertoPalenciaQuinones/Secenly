...
use crate::domain::{License, LicenseAsn1};
...
impl EncapsulateService {
    ...
    pub fn encapsulate_license(license: License, 
        ...
        let content_info = Self::sign_license_der(
            der_bytes, 
            cert_path, 
            key_path)?;

        // Escribir la licencia en der como un archivo
        Self::write_license_der(license_path, &content_info)?;

        Ok(content_info)
    }

    // Firma la licencia, generando el SignedData y la encapsula en un ContentInfo
    fn sign_license_der(license_der_bytes: Vec<u8>, 
                   cert_path: &PathBuf, 
                   key_path: &PathBuf
                   ) -> Result<Vec<u8>, LicenseError> {
